...
class toolbar(Frame):
    def __init__(self, parent, controller, root):
        ...
        mytoolbar = Frame(self.master,bd=1, relief=RAISED)
        #sep = ttk.Separator(toolbar) 
        ## New file 
        self.new_img = ImageTk.PhotoImage(Image.open(os.path.join(os.path.dirname(__file__), r"icons\new-document.png")))
        NewFile = Button(mytoolbar, image=self.new_img, relief=FLAT) ##New file add command
        ...
        OpenFile = Button(mytoolbar, image=self.open_img, relief=FLAT) ##open file add command
        ...
        SaveFile = Button(mytoolbar, image=self.save_img, relief=FLAT) ##save file add command
        SaveFile.grid(column=2,row=0,padx=2, pady=2)
        
        ttk.Separator(mytoolbar,orient='vertical').place(x=78, y=0, height=self.root.winfo_screenwidth())

        ## print file
        self.print_img = ImageTk.PhotoImage(Image.open(os.path.join(os.path.dirname(__file__), r"icons\printer.png")))
        PrintFile = Button(mytoolbar, image=self.print_img, relief=FLAT) ##print file add command
        PrintFile.grid(column=3,row=0,padx=2, pady=2)

        ttk.Separator(mytoolbar,orient='vertical').place(x=104, y=0, height=self.root.winfo_screenwidth())
        

        ## cut file
        self.cut_img = ImageTk.PhotoImage(Image.open(os.path.join(os.path.dirname(__file__), r"icons\cut.png")))
        CutFile = Button(mytoolbar, image=self.cut_img, relief=FLAT) ##cut file add command
        ...
        CopyFile = Button(mytoolbar, image=self.copy_img, relief=FLAT) ##copy file add command
        ...
        PasteFile = Button(mytoolbar, image=self.paste_img, relief=FLAT) ##paste file add command
        PasteFile.grid(column=6,row=0,padx=2, pady=2)

        ttk.Separator(mytoolbar,orient='vertical').place(x=181, y=0, height=self.root.winfo_screenwidth())

        ## delete file
        self.delete_img = ImageTk.PhotoImage(Image.open(os.path.join(os.path.dirname(__file__), r"icons\trash.png")))
        DeleteFile = Button(mytoolbar, image=self.delete_img, relief=FLAT) ##delete file add command
        ...
        UndoFile = Button(mytoolbar, image=self.undo_img, relief=FLAT) ##undo file add command
        UndoFile.grid(column=8,row=0,padx=2, pady=2)

        ttk.Separator(mytoolbar,orient='vertical').place(x=234, y=0, height=self.root.winfo_screenwidth())

        mytoolbar.grid(column=0,row=0, sticky='w')
        root.config(menu=mytoolbar)  